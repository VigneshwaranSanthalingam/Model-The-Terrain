...
#define NODE_4 (Center_white_line>0x60)
#define PATH (Center_white_line>THRESHOLD)
#define NODE (NODE_1||NODE_2||NODE_3||NODE_4)
...
volatile unsigned long int ShaftCountLeft1 = 0;
volatile unsigned long int ShaftCountRight1 = 0;
int x,y,p,q,l,r,SHARP_IR,x1,p1;
int pos = 0;
...
ISR(INT5_vect)
{
  ...
  if (PORTA == 0X06)
  {
  ShaftCountRight1++; //increment right shaft position count
  }
}
...
ISR(INT4_vect)
{
  ...
  if (PORTA == 0X06)
  {
    ShaftCountLeft1++;  //increment left shaft position count
  }
}
...
{  
  lcd_cursor(1,15);
   ...
   while(1)
    ...
    lcd();
    getinput_WL();
    if (NODE_4 || (ShaftCountLeft1 == 65)||(ShaftCountRight1 == 65))
    {
      ShaftCountLeft1 = 0;
      ShaftCountRight1 = 0;
    flag_1 = 1;
    ...
    if(PATH)
    {
      lcd_print(2,15,ShaftCountLeft1,2);
      lcd_print(1,15,ShaftCountRight1,2);
      forward();
      velocity(speed,speed);
      if((ShaftCountLeft1>=28 && ShaftCountLeft1<=47)||(ShaftCountRight1>=28 && ShaftCountRight1<=47))
      {
        getinput_sharp();
    if (SHARP_IR<700)
    {
